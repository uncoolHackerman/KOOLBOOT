...
BPB_SectorsPerCluster: .byte 4  # 2kb clusters
...
BPB_RootDirEntries: .word 512   # value obtained from mkfs.fat
BPB_TotalSectors: .word 40960   # This value isn't used in the KOOLBOOT FAT driver
BPB_MediaType: .byte 0xF8       # I don't know either
BPB_SectorsPerFAT: .word 40     # value obtained from mkfs.fat, 20KiB per FAT
BPB_SectorsPerTrack: .word 32   # what the BIOS would give us for a 20MiB hard drive
BPB_HeadCount: .word 4          # needed for the LBA to CHS conversion
BPB_HiddenSectors: .long 0      # LBA of the start of the partition. Only needed for partitioned hard drives
BPB_LargeSectors: .long 0       # only needed if there are more than 65535 sectors on the disk
# Extended Boot Record (EBR)
EBR_DriveNumber: .byte 0x80     # store dl here
...
EBR_SerialNumber: .long 0x20AC9C0C  # not important, got this from a random number generator
EBR_VolumeID: .ascii "K BOOT BEST"
EBR_SystemID: .ascii "FAT16   " # documentation says not to trust this value but you might as well
...
main:
    ...
    ReadFileLoop:
        ...
        movb (BPB_SectorsPerCluster), %cl   # count = Sectors per cluster
        ...
        movw (%si), %ax         # ax = next cluster
        cmp $0xFFF8, %ax        # check for end of file
        jb ReadFileLoop         # continue if its not the end of the file
        Finish:
